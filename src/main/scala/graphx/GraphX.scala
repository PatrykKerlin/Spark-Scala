...
object GraphX {
    ...
    private def parseNames(line: String): Option[(VertexId, String)] = {
    ...
    private def makeEdges(line: String): List[Edge[Int]] = {
    ...
    def main(args: Array[String]): Unit = {
        ...
        val names = sc.textFile(dfs_path + "Marvel-Names.txt")
        val verts = names.flatMap(parseNames)

        // Build up our edges
        val lines = sc.textFile(dfs_path + "Marvel-Graph.txt")
