...
object DegreesOfSeparation {
    ...
    private val startCharacterID = 5306 //SpiderMan
    private val targetCharacterID = 14 //ADAM 3,031 (who?)

    // We make our accumulator a "global" Option so we can reference it in a mapper later.
    private var hitCounter: Option[LongAccumulator] = None

    // Some custom data types
    // BFSData contains an array of hero ID connections, the distance, and color.
    private type BFSData = (Array[Int], Int, String)
    ...
    private def convertToBFS(line: String): BFSNode = {
    ...
    private def createStartingRdd(sc: SparkContext): RDD[BFSNode] = {
        val inputFile = sc.textFile(dfs_path + "Marvel-Graph.txt")
        inputFile.map(convertToBFS)
    }

    /** Expands a BFSNode into this node and its children */
    private def bfsMap(node: BFSNode): Array[BFSNode] = {
    ...
    private def bfsReduce(data1: BFSData, data2: BFSData): BFSData = {
