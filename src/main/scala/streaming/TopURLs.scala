...
object TopURLs {
    ...
    def main(args: Array[String]): Unit = {
        ...
        val logsDF = accessLines.select(regexp_extract(col("value"), hostExp, 1).alias("host"),
                                        ...
                                        regexp_extract(col("value"), contentSizeExp, 1).cast("Integer")
                                            .alias("content_size"))

        val logsDFWithCurrentTimestamps = logsDF.withColumn("eventTime", current_timestamp())

        // Keep a running count of endpoints
        val endpointCounts = logsDFWithCurrentTimestamps
            .groupBy(window(col("eventTime"), "30 seconds", "10 seconds"), col("endpoint"))
            .count()

        val sortedEndpointsCounts = endpointCounts.orderBy(col("count").desc)

        // Display the stream to the console
        val query = sortedEndpointsCounts
            .writeStream
            .outputMode("complete")
            .format("console")
            .queryName("counts")
