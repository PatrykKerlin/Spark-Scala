...
object MovieSimilarities1M {
    ...
    def main(args: Array[String]): Unit = {
        ...
        val movieNames = spark.sql("SELECT movieID, movieTitle FROM udemy.MovieTitles").as[MoviesNames]

        // Load up movie data as dataset
        val ratings = spark.sql("SELECT userID, movieID, rating FROM udemy.MovieRatings").as[Movies]

        println(s"Before self-join: ${ratings.count()} rows")
        ...
        println(s"After self-join: ${moviePairs.count()} rows")
